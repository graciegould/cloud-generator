const calculateCloudDensity = (relativeHumidity, airTemp, airPressure) => {
  ...
  const saturationVaporPressure =
    6.112 * Math.pow(10, (7.5 * airTemp) / (237.3 + airTemp));
...
const calculateDewPointFromHumidity = (airTemp, relativeHumidity) => {
  ...
  const b = 243.12;
  const alpha =
    (a * airTemp) / (b + airTemp) + Math.log(relativeHumidity / 100);
...
const calculateDewPointFromAltitude = (
  altitude,
  baseDewPoint,
  baseAltitude,
) => {
  ...
  const newDewPoint =
    baseDewPoint - (altitude - baseAltitude) * dewPointLapseRate;
